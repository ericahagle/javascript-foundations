...
describe('video games', function () {
	...
	it('should create a new player with a unique moveset', function () {
		var player1 = createPlayer('Super Skater', 27, [
			'super jump',
			'board smash',
			'grind rail'
		]);
		...
		assert.deepEqual(
			player1.moveset[('super jump', 'board smash', 'grind rail')]
		);
	});
	...
	it('should be able to create a new level with a name and a player', function () {
		var player1 = createPlayer('The Future Savior', 18, [
			'time travel',
			'combat training',
			'futuristic weapon'
		]);
	...
	it('should be able to create a new level with a name and many players', function () {
		var player1 = createPlayer('Rock Star Rebel', 21, [
			'power chord',
			'stage dive',
			'crowd surf'
		]);
		var player2 = createPlayer('Pro Skater Prodigy', 16, [
			'360 flip',
			'board slide',
			'gap transfer'
		]);
	...
	it('should initiate a level with a player and 3 lives and 0 coins', function () {
		var player1 = createPlayer('Quantum Jumper', 30, [
			'dimension jump',
			'portal creation',
			'alternate reality explosion'
		]);
	...
	it('should be able to collect coins', function () {
		var player1 = createPlayer('The Timeless Troubadour', 500, [
			'piano sonata',
			'violin concerto',
			'fllamenco guitar'
		]);
	...
	it('should add a life when 100 coins are collected', function () {
		var player1 = createPlayer('The Fiery Friar', 50, [
			'flame spin',
			'fire dance',
			'smoke screen'
		]);
	...
	it('should be able to defeat the player and cost a life', function () {
		var player1 = createPlayer('Tony Thrasher', 34, [
			'thrash throw',
			'solving mysteries',
			'thrash dance'
		]);
	...
	it('should print `GAME OVER` when a player is defeated and has no lives left', function () {
		var player1 = createPlayer('Ant Cat', 3, [
			'tiny roar',
			'six-legged claws',
			'dig tunnel'
		]);
		...
		var updatedLevelPlayerAnnihilated = defeatPlayer(
			updatedLevelPlayerDefeated
		);
		assert.equal(updatedLevelPlayerAnnihilated.lives, 1);
		var updatedLevelPlayerDestroyed = defeatPlayer(
			updatedLevelPlayerAnnihilated
		);
